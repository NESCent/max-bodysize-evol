...
module GoogleChart
  ...
  class LineChart < Base
    attr_accessor :is_xy,:display_lines
    ...
    def add_display_line(lines)
      self.display_lines+=lines
    end
    
    def add_sub_chart(lines)
      @sub_charts<<lines
    end
      ...
      if self.is_xy or @data.size > 1
        if self.is_xy # XY Line graph data series
          ...
          @data.size.times { |i|
            ...
            if self.display_lines.size>0 
              if self.display_lines.include?(@labels[i])
                encoded_data << join_encoded_data([encode_data(x_data[i],max_x_value,min_value), encode_data(y_data[i],max_y_value,min_value)])
              end
            else
              encoded_data << join_encoded_data([encode_data(x_data[i],max_x_value,min_value), encode_data(y_data[i],max_y_value,min_value)])
            end
    ...
    def process_data_subchart(index)
      if self.is_xy or @data.size > 1
        if self.is_xy # XY Line graph data series
          encoded_data = []
          @data.size.times { |i|
            # Interleave X and Y co-ordinate data
            if @sub_charts[index].size>0 
              if @sub_charts[index].include?(@labels[i])
                encoded_data << join_encoded_data([encode_data(x_data[i],max_x_value,min_x_value), encode_data(y_data[i],max_y_value,min_y_value)])
              end
            else
              encoded_data << join_encoded_data([encode_data(x_data[i],max_x_value,min_x_value), encode_data(y_data[i],max_y_value,min_y_value)])
            end
          ...
          join_encoded_data(@data.collect { |series|
                              encode_data(series, max_data_value,min_data_value)
                            })
        end
      else
        encode_data(@data.flatten, max_data_value,min_data_value)
      end
    end
  end
end
