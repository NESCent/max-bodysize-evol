...
module GoogleChart
  class Base
    ...
    def range(values)  # Added by Adam 11/08
      case data_encoding
      when :simple
        values = simple_encode(values)
      when :text
        values = text_encode_each_value(values)
      when :extended
        values = extended_encode(values)
      else
    ...
    # Generates the URL string for the subchart that can be used to retrieve the graph image in PNG format.
    # Use this after assigning all the properties to the graph
    # You can pass in additional params as a hash for features that may not have been implemented
    # For e.g
    #      lc = GoogleChart::LineChart.new('320x200', "Line Chart", false)
    ...
    #      lc.data "Trend 3", [6,5,4,3,2,1], 'ff0000'
    #      puts lc.to_url({:chm => "000000,0,0.1,0.11"}) # Single black line as a horizontal marker        
    def to_url_subchart(index,extras={})
      prepare_params_subchart(index)
      params.merge!(extras)
      query_string = params.map { |k,v| "#{k}=#{URI.escape(v.to_s).gsub(/%20/,'+').gsub(/%7C/,'|')}" }.join('&')
      BASE_URL + query_string
    end
    ...
    # Allows (optional) setting of a min value for the chart, which will be used for data encoding and axis plotting.
    # The value to pass depends on the type of chart
    # * For Line Chart and Bar Charts it should be a single integer or float value
    # * For Scatter Charts and Line XY Charts, you MUST pass an array containing the minimum values for X and Y
    # 
    # ==== Examples
    # For bar charts
    #    bc.mix_value 5 # 5 will be used to calculate the relative encoding values
    # For scatter chart
    #    sc.mix_value [5,6] # 5 is the max x value and 6 is the max y value
    #
    # Note : MAKE SURE you are passing the right values otherwise an exception will be raised
    def min_value(value)
      if [:lxy, :s].member?(self.chart_type) and value.is_a?(Array)
        ...
        raise "Invalid min value for this chart type"
      end
    end
    ...
    def grid(options={})
      ...
      if options[:x_offset]
        x_offset=options[:x_offset].to_f
      end
      
       if options[:y_offset]
        y_offset=options[:y_offset].to_f
      end
      
      @grid_str += ",#{x_offset},#{y_offset}"
    end
    ...
    def prepare_params
     ...
     def prepare_params_subchart(index)
      params.clear
      set_size
      set_type
      set_colors
      set_fill_options_subchart(index)
      add_axis unless @axis.empty?
      add_grid  
      add_data_subchart(index)
      set_range
      add_line_styles unless @line_styles.empty?
      set_bar_width_spacing_options if @bar_width_spacing_options
      add_markers unless @markers.empty?
    ...
    def set_fill_options_subchart(index)
      if index>0
        @background_fill="bg,s,65432100"
      end
      fill_opt = [@background_fill, @chart_fill].compact.join("|")
      params.merge!({:chf => fill_opt}) if fill_opt.length > 0
    end
    ...
    def set_range # Added by Adam 11/08
      params.merge!({:chds => @range.join(",")}) unless @range.empty?
    end
    ...
    def add_data_subchart(index)
      converted_data = process_data_subchart(index)
      case data_encoding
      when :simple
        converted_data = "s:" + converted_data
      when :text
    ...
    def encode_data(values, max_value=nil,min_value=nil)
      case data_encoding
      when :simple
        simple_encode(values, max_value,min_value)
      when :text
        text_encode(values, max_value,min_value)
      when :extended
        extended_encode(values, max_value,min_value)
      else
        raise "Illegal Encoding Specified"
      end
    end
    
    def simple_encode(values,  max_value=nil, min_value=nil)
      alphabet_length = 61
      max_value = values.max unless max_value
      min_value = values.min unless min_value
      range=max_value-min_value
      if range.to_i==0
        raise "Illegal max and min for encoding Google chart data"
      end
      chart_data = values.collect do |val|              
        if val
          SIMPLE_ENCODING[(alphabet_length * (val-min_value) / range).to_i]
        else
          "_"
        end
      end
    ...
    def text_encode(values, max_value=nil, min_value=nil)
      max_value = values.max unless max_value
      min_value = values.min unless min_value
      range=max_value-min_value
      if range.to_i==0
      ...
      values.inject("") { |sum, v|
        sum += ( "%.1f" % ((v-min_value)*100/range) ) + ","
      }.chomp(",")
    end
    
    
    def text_encode_each_value(values, max_value = nil) # Added by Adam 11/08
      max_value ||= values.max
      encoded = values.collect { |v| ( "%.1f" % (v*100/max_value)) }
      return encoded
    end
    
    
    def extended_encode(values, max_value=nil, min_value=nil)
      max_value = values.max unless max_value
      min_value = values.min unless min_value
      range=max_value-min_value
      if range.to_i==0
      ...
      values.collect { |v|
        @@complex_encoding[((v-min_value) * 4095/range).to_i]
      }.join('')
    end
    ...
    def min_data_value
      @min_data or @data.flatten.min
    end
    
    def min_x_value
      @min_x or x_data.flatten.min
    end

    def min_y_value
      @min_y or y_data.flatten.min
    end
